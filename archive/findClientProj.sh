...
if [ $# -ne 2 ]; then
    echo need two arguments as package prefix and library name!
    exit 1
fi

arg=$1  # "org.apache.ibatis"
libName=$2  # mybatis
...
for i in {1..10}; do
    while [ ! -f $resultDir/$i.json ] || file_contains_string $resultDir/$i.json "exceeded a secondary rate limit" || [ `cat "$resultDir/$i.json" | grep \"score\": | wc -l` -lt 90 ]; do
        echo searching for the $i page
        curl -D $resultDir/header.log "https://api.github.com/search/code?q=\"import+$arg\"+language:java&per_page=100&page=$i" \
        -H "Accept: application/vnd.github.v3+json" \
